    ...
    $.get(url, function (result) {
        ...
        var mesta = result.response.venues;
        $('#display').append(mesta);
        for (var i in mesta){
            mesto = mesta[i];
            var str = '<p><strong>' + mesto.name + '</strong> '; 
            str += '</p>';
            $('#display').append(str);
            // place the a marker on the map
            marker = new google.maps.Marker({
                position: new google.maps.LatLng(mesto.location.lat,mesto.location.lng),
                map: map
            });
        }});
